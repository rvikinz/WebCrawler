...
/*
 * Main Function to call is - public static boolean robotsShouldFollow( String url )
 */
...
public class RobotExclusionUtil 
{
	...
	private static boolean parseRobotsFileString(String site, String robotsFile) 
	{
		int currentIndex = 0;
		...
		ConcurrentHashMap<String, Boolean> m = ThreadPool.mapSites.get(site);
		if (m == null) 
		{
			m = new ConcurrentHashMap<String, Boolean>();
			ThreadPool.mapSites.put(site, m);
		}
	...
	public static boolean robotsShouldFollow(String url) 
	{
		...
		try 
		{
			...
			if (! ThreadPool.mapSites.containsKey(site)) 
			{
				String robotText = readRobotsFile(url("http://" + site + "/robots.txt"));
				
				// robots.txt file Exist
				if (robotText != null) 
				{
					//System.out.println(robotText);
					isCrawlingAllowed = parseRobotsFileString(site, robotText);
					isCrawlingAllowed = true;
					if( !isCrawlingAllowed )
						System.out.println("Crawling not Allowed for this URL ---->> " + site);
				}
			}
			else
			{
				//Adding code for Disallow folders of particular websites in robots.txt
				ConcurrentHashMap<String, Boolean> tempMap = ThreadPool.mapSites.get( site );
				if( tempMap == null )
					return true;
				...
				if( path != null && path.length() > 0 && path.endsWith("/") )
				{
					...
					for( Map.Entry<String, Boolean> entry: tempMap.entrySet() )
					{
						...
						if( path.toLowerCase().contains( tempStr.toLowerCase() ) )
							isCrawlingAllowed = tempMap.get( tempStr );
					}
				}
			}
	...
	public static String readRobotsFile(URL urlObj) throws IOException {
		...
		// using a StringBuffer instead of a String has huge performance benefits.
		...
		*/
		...
		// make up a name where you would store the open a file to write in
		String line = "";
